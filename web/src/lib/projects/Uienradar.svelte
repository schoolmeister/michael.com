<script lang="ts">
	...
	onMount(async () => {
		if (browser) {
			// Use the default (CJS) export: plugins mutate that live object at runtime,
			// while the frozen ESM namespace in prod builds only has build-time exports.
			const leafletModule = (await import('leaflet')) as typeof import('leaflet') & {
				default?: typeof import('leaflet');
			};
			L = leafletModule.default ?? leafletModule;
			// ESM build exports the control as a class instead of attaching L.control.locate
			({ LocateControl } = await import('leaflet.locatecontrol'));
			...
			// Observe element size changes (drag-resize/maximize of custom Window does NOT trigger window.resize)
